...
## 1. Habitat Integration (`src/nyra/integration.ts`)
...
- **`sendMemoryToNyra`**: Im Mock-Modus wird nur eine Konsolenausgabe erzeugt; es erfolgt keine echte Speicherung oder Übertragung.
...
- **STATUS: REAL (OpenRouter Integration)**: Die Bridge nutzt nun OpenRouter (Vision-Modelle), um echte 3s-Movie-Extraktionen autonom zu analysieren und an Nyra zu senden. Der `localFallback` bleibt nur als Ausfallsicherheit aktiv.
